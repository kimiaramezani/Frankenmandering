...
class FrankenmanderingEnv(gym.Env):
    ...
    def reset(self, seed: int | None = None, options: dict | None = None, mode = None):
        super().reset(seed=seed)

        if mode is None:
          self.FrankenData = self.initial_data
          self.committed_state = self.initial_data
          self.t = 0
          self.transition_history = []

        else :
          self.FrankenData = self.committed_state
    ...
    def commit(self, committed_state):
      ...
      self.FrankenData = committed_state
