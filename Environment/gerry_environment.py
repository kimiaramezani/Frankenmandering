...
class FrankenData(Data):
    ...
    def set_number(self, orig_edge_num):
      self._orig_edge_num = orig_edge_num
    ...
    def set_geo_edge(self, geo_edge):
      self._geographical_edge = geo_edge
    ...
    def set_geo_edge_attr(self,edge_attr):
      self._geo_edge_attr = edge_attr
...
class FrankenmanderingEnv(gym.Env):
    ...
    def step(self, action, DRF):
        ...
        # assignment = labels_to_action(action, self._num_districts)
        dist_label = action.argmax(axis=1)
    ...
    def reward(self, new_opinion):
        ...
        init_ops = self._initial_data.get_x()
        new_d = torch.linalg.norm(new_opinion-init_ops)
