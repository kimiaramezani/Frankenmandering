...
def opinion_update(edge_index_aug, edge_attr, opinion: np.ndarray,drf):
...
def population_equality(pop_weights, dist_label, num_districts):
    ...
    pop_per = np.zeros(num_districts, dtype=pop_weights.dtype, device=pop_weights.device)
    for d in range(num_districts):
...
def compactness_score(geo_edge: torch.Tensor, dist_label: torch.Tensor) -> float:
    """
