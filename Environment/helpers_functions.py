# -*- coding: utf-8 -*-
"""Helpers_functions_6.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1LKJH8Xo6nvYjsPgu8cRHOMiuJHs-BDdc
"""
...
def augment_with_reps(edge_index, edge_attr, reps, dist_label, rep_edge_weight: float = 1.0):
    ...
    for d, r in enumerate(reps):
        ...
        for v in members:
            ...
            add_dst.append(torch.tensor(v, dtype=edge_index.dtype))
            add_w.append(torch.tensor(rep_edge_weight, dtype=edge_attr.dtype)) 

    if not add_src:  # no additional edges
        return edge_index, edge_attr


    add_src = torch.stack(add_src)
    add_dst = torch.stack(add_dst)
    add_w   = torch.stack(add_w)

    rep_edges = torch.vstack([add_src, add_dst])
    rep_weights = add_w
...
def population_equality(pop_weights, dist_label, num_districts):
    pop_per = torch.zeros(num_districts)
    for d in range(num_districts):
        pop_per[d] = pop_weights[dist_label == d].sum()

    ideal = (pop_per.sum()) / num_districts
    pop_dev = torch.abs(pop_per - ideal).sum() / (ideal * num_districts)
    return pop_dev
