...
class FrankenmanderingEnv(gym.Env):
    ...
    def step(self, action: np.ndarray, eps_assim, eps_backfire):
        ...
        # Update oppinion
        x_new = opinion_update(self.opinion_update_step, aug_edge_index, aug_edge_attr,self.FrankenData.opinion, eps_assim, eps_backfire)
...
def opinion_update(edge_index_aug, edge_attr, opinion: np.ndarray,eps_assim , eps_backfire):
    ...
    for e in range(edge_index_aug.shape[1]):
       ...
       # if diff is a 1-D array [x, y, z], then np.linalg.norm(diff, 2) calculates sqrt(x^2 + y^2 + z^2)
        ...
        if dist > 1e-8:  # avoid division by zero
            ...
            mu = drf(dist, eps_assim, eps_backfire, assim_shift, back_shift)  # influence factor
...
def drf(discrepancy, eps_assim, eps_backfire, assim_shift, back_shift):
    ...
    elif delta >= eps_backfire: #backfire
          return back_shift
    else:
